...
unsigned char DHT11_READ_DATA(void)
{
    ...
    if(DHT11_Check())                               //  ??DHT11??
    {
        ...
        if(data[0] + data[1] + data[2] + data[3] == data[4])
        {
            ...
            OLED_ShowNum(98,0,data[2],2,16);
//						OLED_ShowString(108,0,"℃",1);

            //湿度显示
            OLED_ShowString(0,2,"Humidity",16);
            OLED_ShowString(65,2,":",16);
            OLED_ShowNum(73,2,data[0],2,16);
//						OLED_ShowString(68,2,"HR",2);
            return 1;                               //  ??????
        }
        else
        {
...
static void DHT11_GPIO_MODE_SET(uint8_t mode)
{
    if(mode)
    {
        ...
        GPIO_InitStruct.Pin = GPIO_PIN_4;                   //  9???
    ...
    else
    {
        ...
        GPIO_InitStructure.Pin = GPIO_PIN_4;                //  9???
